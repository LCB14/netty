...
public abstract class ReferenceCountUpdater<T extends ReferenceCounted> {
    ...
    private static int realRefCnt(int rawCnt) {
        /**
         * 奇偶判断
         * 判断一个数是奇数还是偶数其实很简单，直接通过  rawCnt & 1 就可以判断，如果返回 0 表示 rawCnt 是一个偶数，如果返回 1 表示 rawCnt 是一个奇数。
         *
         * 其实 Netty 这里是为了尽量用性能更高的 == 运算来代替 & 运算，但又不可能用 == 运算来枚举出所有的偶数值（也没这必要），
         * 所以只用 == 运算来判断在实际场景中经常出现的引用计数，一般经常出现的引用计数值为 2 或者 4 ，
         * 也就是说 ByteBuf 在大部分场景下只会被引用 1 次或者 2 次，对于这种高频出现的场景，Netty 用 == 运算来针对性优化，低频出现的场景就回退到 & 运算。
         */
    ...
    public final int refCnt(T instance) {
        // 通过 updater 获取 refCnt，根据 refCnt 在  realRefCnt 中获取真实的引用计数
